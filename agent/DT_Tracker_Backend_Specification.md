...
> **Last updated:** July 2026 — firmware adaptive reporting, UTC timestamp semantics, deployment prerequisites, and Swagger added
...
# 41. Firmware Integration — Current Implementation

The current DT Tracker firmware runs as a PlatformIO Arduino project using:
...
```text
DT-Tracker-Device/src/main.cpp
```

## 41.1 Backend environment selection

Exactly one firmware environment must be active:
...
The resolved URLs are:

| Firmware environment | Ingest URL |
|---|---|
| Development | `https://dev.dt-tracker.com/api/v1/ingest` |
| Staging | `https://staging.dt-tracker.com/api/v1/ingest` |
| Production | `https://api.dt-tracker.com/api/v1/ingest` |

The device must never contain:

- Firebase service-account credentials.
- Firebase user credentials.
- `ADMIN_API_KEY`.

The current `/ingest` endpoint does not yet implement per-device authentication.

## 41.2 Current telemetry payload
...
`datetime` is generated from GNSS and is stored in UTC.

A value such as:

```text
2026-07-13T17:29:34.000Z
```

is correct UTC time. Flutter must convert it for presentation:

```dart
final localTime = DateTime.parse(value).toLocal();
```

`ts` is currently based on ESP32 `millis()`:

```cpp
event.ts = millis();
```

It is not a Unix timestamp. It is useful as a device uptime/ordering value and must not be rendered as a calendar date.

## 41.3 Adaptive GPS and upload policy

The recommended firmware separates GNSS polling from backend uploading.

| Condition | Recommended behavior |
|---|---:|
| GNSS polling | Every 5 seconds |
| Moving at 5 km/h or more | Upload every 10 seconds |
| Moving at 1–5 km/h | Upload every 30 seconds |
| Stationary below 1 km/h | Upload every 2 minutes |
| Maximum heartbeat | Every 5 minutes |
| Distance from last successful upload | Force upload at 25 meters |

Recommended constants:

```cpp
constexpr uint32_t GPS_POLL_INTERVAL_MS = 5000;
constexpr uint32_t MOVING_UPLOAD_INTERVAL_MS = 10000;
constexpr uint32_t SLOW_UPLOAD_INTERVAL_MS = 30000;
constexpr uint32_t STATIONARY_UPLOAD_INTERVAL_MS = 120000;
constexpr uint32_t MAX_HEARTBEAT_INTERVAL_MS = 300000;

constexpr float MOVING_SPEED_KMH = 5.0F;
constexpr float STATIONARY_SPEED_KMH = 1.0F;
constexpr float MIN_DISTANCE_TO_FORCE_SEND_METERS = 25.0F;
```

This provides responsive live tracking and geofence detection without uploading every GNSS sample while parked.

## 41.4 Offline behavior

The firmware maintains an in-memory queue of up to 20 telemetry records.

Current limitation:

- The queue is lost after a restart or power loss.
- A future version should persist buffered telemetry using LittleFS, NVS, or external storage.

The device flushes one buffered record per loop so that GPS polling is not blocked for long periods.

## 41.5 Watchdog compatibility

PlatformIO Espressif32 `6.7.0` uses an older Arduino/ESP-IDF generation. The firmware must use watchdog APIs compatible with that platform or conditional compilation based on `ESP_IDF_VERSION_MAJOR`.

---

# 42. Required Data and Resources Before Flutter Integration

Not every collection must be pre-populated. The minimum required data depends on the feature being tested.

## 42.1 Environment resources that must already exist
...
- Matching PM2 process and Nginx route.

## 42.2 Required backend environment values
...
ADMIN_API_KEY=<production-secret>
```

## 42.3 Minimum data for tracker validation

Required:

```text
RTDB: trackers_registry/{imei}
```

The tracker should normally be created through:

```http
POST /api/v1/trackers/register
```

Do not manually create ownership values unless recovering corrupted data.

## 42.4 Minimum data for linking

Required before calling the link endpoint:
...
The vehicle must belong to the authenticated UID.

The tracker registry entry must exist and be available, or be idempotently associated with the same owner depending on backend behavior.

The link endpoint creates these resources automatically:

```text
RTDB users/{uid}/devices/{imei} = true
RTDB trackers_registry/{imei}/ownerId
RTDB trackers_registry/{imei}/vehicleId
RTDB trackers_registry/{imei}/status = linked
RTDB trackers_registry/{imei}/linkedAt
Firestore users/{uid}/vehicles/{vehicleId}/trackerId
Firestore users/{uid}/vehicles/{vehicleId}/trackerLinkedAt
```

## 42.5 Minimum data for telemetry

No Firestore user or vehicle is required merely to persist telemetry.

The ingest endpoint can write:
...
However, geofence evaluation requires a linked tracker registry entry containing:
...
## 42.6 Minimum data for geofence processing
...
If `geofenceAlertEnabled` is false, state transitions still occur but events are suppressed.
...
The first observed position initializes state and intentionally creates no event.

## 42.7 Minimum data for Flutter live tracking
...
RTDB users/{uid}/devices/{imei} = true
RTDB trackers_live/{imei}
```

The user-device mapping should be created only by the backend link endpoint.

## 42.8 Minimum data for event timeline

At least one backend-generated document under:

```text
users/{uid}/events/{eventId}
```

No event documents should be seeded manually in production. For staging, test events should preferably be generated by real ingest transitions.

## 42.9 Firestore composite indexes

Required for geofence evaluation:
...
Potentially required for the Flutter unread event list:
...
Indexes must be created independently in each named Firestore database.

---

# 43. Final Security Rules — Current Version

## 43.1 Firestore vehicle ownership fields

Flutter may manage vehicle business data but may not directly create or update backend-owned tracker linkage fields:
...
A linked vehicle may not be deleted until it is unlinked through the backend.

## 43.2 Event status consistency

Flutter may only produce these valid combinations:

```text
status = new       -> isRead = false
status = read      -> isRead = true
status = archived  -> isRead = true
```

## 43.3 Storage behavior

Current Storage rules:

- Permit `get`, not directory listing.
- Permit create/update only for images up to 10 MB.
- Permit delete for the owner.
- Deny all other paths.

## 43.4 RTDB history reads

Flutter must read history at the date level:

```text
trackers_history/{imei}/{YYYY-MM-DD}
```

The current RTDB rules do not grant a read at:

```text
trackers_history/{imei}
```

---

# 44. Deployment Prerequisites Per Environment

Before promoting the backend and firmware:

## Staging

```text
[ ] .env.staging points to staging RTDB and Firestore
[ ] ADMIN_API_KEY configured
[ ] Firestore rules deployed
[ ] RTDB rules deployed
[ ] Storage rules deployed
[ ] Composite indexes enabled
[ ] Staging Auth user exists
[ ] Staging Firestore user exists
[ ] Staging vehicle exists
[ ] Staging tracker registered
[ ] PM2 staging process online
[ ] Nginx staging domain works
[ ] Firmware ENV_STAGING selected
[ ] Device returns HTTP 200
[ ] Telemetry appears only in staging
[ ] Link/unlink succeeds
[ ] Geofence enter/exit creates events
```

## Production

```text
[ ] Exact staging-tested commit selected
[ ] Production backup completed
[ ] .env.prod verified
[ ] Production rules deployed
[ ] Production indexes enabled
[ ] Real tracker registered
[ ] Production PM2 process restarted
[ ] Health checks pass
[ ] Firmware ENV_PROD selected
[ ] Controlled smoke test succeeds
...
The companion OpenAPI file is:

```text
openapi-dt-tracker.yaml
```
...
- Tracker validation.
- Vehicle tracker link.
- Vehicle tracker unlink.
- Authentication schemes.
- Request/response examples.
- Common errors.

Firestore, RTDB, and Storage direct-client operations are documented in this Markdown specification because they are Firebase SDK contracts, not REST endpoints in the current backend.
